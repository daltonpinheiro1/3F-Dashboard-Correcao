...
"$RG" -qF 'aba="advertencias"' src/App.tsx || fail "Advertências deve usar AuthGuard aba=advertencias"
"$RG" -qF 'aba="atestados-solicitar"' src/App.tsx || fail "solicitar atestado deve usar AuthGuard aba=atestados-solicitar"
"$RG" -qF "abaId: 'advertencias'" src/components/AdminLayout.tsx || fail "sidebar Advertências deve ter abaId advertencias"
"$RG" -qF "abaId: 'atestados-solicitar'" src/components/AdminLayout.tsx || fail "sidebar Solicitar atestado deve ter abaId atestados-solicitar"
...
"$RG" -q "export async function decodeAtestadoImage" src/lib/atestadosImagePrep.ts || fail "decodeAtestadoImage ausente (HEIC/Pixel)"
"$RG" -q "createImageBitmap" src/lib/atestadosImagePrep.ts || fail "prep deve decodificar HEIC/AVIF via createImageBitmap"
"$RG" -q "export function sniffAtestadoMagic" src/lib/atestadosStorage.ts || fail "sniff magic JPEG/HEIC ausente (câmera Android)"
"$RG" -q "export function resolveAtestadoKind" src/lib/atestadosStorage.ts || fail "resolveAtestadoKind ausente (magic vence extensão)"
"$RG" -q "resolveAtestadoKind" src/components/atestados/ProtocolarPanel.tsx || fail "ProtocolarPanel deve resolver tipo pelo magic"
"$RG" -q "sniffAtestadoMagic" src/components/atestados/ProtocolarPanel.tsx || fail "ProtocolarPanel deve sniffar JPEG/HEIC da câmera"
"$RG" -q "Tirar foto" src/components/atestados/CapturaGuiada.tsx || fail "CapturaGuiada deve ter botão Tirar foto"
"$RG" -q "Galeria / PDF" src/components/atestados/CapturaGuiada.tsx || fail "CapturaGuiada deve ter Galeria / PDF"
"$RG" -q "accept=\"image/\*\"" src/components/atestados/CapturaGuiada.tsx || fail "input câmera deve aceitar image/*"
"$RG" -q "capture=\"environment\"" src/components/atestados/CapturaGuiada.tsx || fail "CapturaGuiada deve ter input câmera (capture environment)"
if "$RG" -q "Imagem inválida" src/lib/atestadosImageQuality.ts; then
  fail "qualidade da foto não pode recusar HEIC com 'Imagem inválida'"
fi
"$RG" -q "magic JPEG vence" src/lib/atestadosStorage.test.ts || fail "teste resolveAtestadoKind JPEG vs .pdf ausente"
...
"$RG" -q "slidesRrApresentacao" src/components/rr/RrWarRoom.tsx || fail "War room deve usar pauta de slides da RR"
"$RG" -q "id: 'pauta'" src/lib/rrVista.ts || fail "apresentação RR deve ter slide Pauta"
"$RG" -q "fraseDaCasa" src/lib/rrCultura.ts || fail "frase da casa ausente"
"$RG" -q "RrFrasePodio" src/pages/RrPage.tsx || fail "RR deve abrir com frase da casa e pódio/banco"
"$RG" -q "id: 'casa'" src/lib/rrVista.ts || fail "TV RR deve abrir no slide Casa"
...
"$RG" -q "requireRr" functions/api/rr-360.ts || fail "rr-360 deve exigir requireRr"
"$RG" -q "requireRr" functions/api/rr-alert-ack.ts || fail "rr-alert-ack deve exigir requireRr"
"$RG" -q "requireRr" functions/api/rr-insight.ts || fail "rr-insight deve exigir requireRr"
...
"$RG" -q "escolherMotivoOperacional" functions/api/portabilidade-funil.ts || fail "funil deve preferir motivo BKO/Toutbox"
"$RG" -q "rotuloIccidPorAndamento" functions/api/portabilidade-funil.ts || fail "funil deve rotular ICCID pelo andamento"
"$RG" -q "classificarFatia" functions/api/portabilidade-funil.ts || fail "funil deve classificar por andamento Toutbox"
"$RG" -q "em rota · aguardando" functions/_lib/portabilidadeAndamento.ts || fail "em rota deve ser espera, não erro"
"$RG" -q "consultar ICCID Toutbox" functions/_lib/portabilidadeAndamento.ts || fail "entregue sem chip deve consultar ICCID na Toutbox"
"$RG" -q "function isEsim" functions/_lib/portabilidadeAndamento.ts || fail "eSIM deve ser detectado (não vai à Toutbox)"
"$RG" -q "acaoMatrixTim" functions/_lib/portabilidadeAndamento.ts || fail "eSIM BKO deve cair na matrix TIM"
"$RG" -q "OS sem ticket" functions/_lib/portabilidadeAndamento.ts || fail "OS sem ticket deve ser consult"
...
"$RG" -q "dropTotalCanonico" src/pages/OperacaoPage.tsx || fail "Operação DROP casa deve ser dropTotalCanonico"
"$RG" -q "dropTotalCanonico" src/pages/DiscagensPage.tsx || fail "Discagens DROP dia deve ser dropTotalCanonico"
"$RG" -q "jornada.length === 0" src/lib/chamadasVisoes.ts || fail "dropTotalCanonico não pode reinflar DROP sob busca"
...
"$RG" -q "resetKey" src/lib/tableSort.ts || fail "useTableSort deve aceitar resetKey (troca de modo da matriz)"
"$RG" -q "tabHoraSortCol" src/pages/DiscagensPage.tsx || fail "matriz tab×hora deve ordenar cada coluna de hora"
...
"$RG" -q "dataBrtIso()" src/components/advertencias/CriacaoPanel.tsx || fail "data da advertência deve ser calendário BRT"
"$RG" -q "_drop_tabs" src/pages/OperacaoPage.tsx || fail "Operação DROP% jornada não pode pintar 0% sem tabs"
"$RG" -F -q "Ofensores P0/P1 ficam no huddle live" src/pages/RrPage.tsx || fail "RR qualidade não pode listar ofensores do dia fora do live"
...
"$RG" -F -q "enriquecerSerieHora" src/lib/mailingVisoes.ts || fail "série horária deve expor expectativa/aderência"
"$RG" -F -q "distDoRecorte" src/lib/mailingVisoes.ts || fail "dist do recorte deve exigir cobertura completa"
"$RG" -F -q "usarLive = tab === 'live'" src/pages/MailingPage.tsx || fail "histórico não pode misturar live no dia corrente"
"$RG" -F -q "useFiltroEvaStore" src/pages/MailingPage.tsx || fail "Mailing deve compartilhar a campanha do filtro EVA"
"$RG" -F -q "CAMPANHA_FILTRO_OPTIONS" src/pages/MailingPage.tsx || fail "Mailing deve exibir chips de produto fixos"
"$RG" -q "requireGestao" functions/api/mailing-indice.ts || fail "/api/mailing-indice exige gestão"
"$RG" -F -q "mailing/dias.json" functions/api/mailing-indice.ts || fail "API índice mailing deve ler dias.json"
"$RG" -F -q "MAILING_INDICE_URL" src/lib/mailingSaude.ts || fail "cliente deve buscar índice em endpoint próprio"

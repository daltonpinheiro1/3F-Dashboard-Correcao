          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <section className="rounded-2xl border border-orange-200 bg-orange-50/70 p-4">
                ...
                <ol className="space-y-2">
                  ...
                  {analise.deslogs.map((d, i) => (
                    <li key={`${d.logout}-${d.relogin}`} className="rounded-xl bg-white border border-orange-200 px-3 py-2">
                      <p className="text-[11px] font-bold uppercase tracking-wide text-orange-700">
                        {houveAtraso || i === 0 ? `Alerta na dela · relogin ${i + 1}` : `Relogin ${i + 1}`}
                      </p>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {tabs.slice(0, 12).map((t) => {
                          ...
                          return (
                            <tr key={`${t.nome}-${t.campanha_op}`} className="border-t border-gray-50">
                              <td className="px-3 py-1.5 text-gray-800">{t.nome}</td>
                ...
                {recs.length === 0 ? (
                  ...
                  <ul className="space-y-1">
                    {recs.slice(0, 12).map((c) => (
                      <li key={c.id} className="flex justify-between gap-2 text-xs border-b border-gray-50 py-1.5">
                        <span className="text-gray-500 tabular-nums">{fmtHora(c.call_time)}</span>
                        <span className="flex-1 truncate text-gray-800">{c.classification_name}</span>
                        <span className={c.success ? 'text-emerald-700' : (c.cpc_op ?? c.cpc) ? 'text-teal-700' : 'text-gray-400'}>
                          {c.success ? 'OK' : (c.cpc_op ?? c.cpc) ? 'CPC' : '—'}
                        </span>
                      </li>
                    ))}
                  </ul>
              ...
              <p className="text-[11px] text-gray-400">
                Tab. {j.tabuladas || 0} · sucesso {j.sucesso || 0} · TMA {fmtHms(j.tma_seg)} · perda deslogue {fmtDur(j.tempo_perdido_seg)}

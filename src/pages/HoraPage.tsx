    ...
    >
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* ─── #11 CPC por campanha ─── */}
          {campanha === 'TODAS' &&
            (cpcPorCamp.port.total > 0 || cpcPorCamp.mig.total > 0 || cpcPorCamp.bko.total > 0 || cpcPorCamp.cc.total > 0 || cpcPorCamp.algar.total > 0) && (
            <div className={`grid gap-4 mb-6 ${(cpcPorCamp.bko.total > 0 || cpcPorCamp.cc.total > 0 || cpcPorCamp.algar.total > 0) ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-2'}`}>
              ...
              {cpcPorCamp.algar.total > 0 && (
                <div className={`card p-4 shadow-sm ${cpcPorCamp.algar.pct < metaDia && cpcPorCamp.algar.total >= 8 ? 'border-red-200 bg-red-50' : ''}`}>
                  <p className="text-[10px] font-semibold uppercase text-gray-400">Algar</p>
                  <p className="text-2xl font-black">{cpcPorCamp.algar.pct}% CPC</p>
                  <p className="text-xs text-gray-500">
                    {cpcPorCamp.algar.vendas} vendas · {cpcPorCamp.algar.total} tab. · recorte próprio (≠ Port TIM)
                  </p>
                </div>
              )}
            </div>
